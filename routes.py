...
def generate_gitlab_issues():
    try:
        ...
        if not os.path.exists(store_path):
            return jsonify({'message': '%s not exist!' % store_path}), 400
        else:
            ...
            db.get_engine().execute("delete from gitissues")
            for issue in all_issues:
                new_issue = GitIssues(settings=str(git_issues_settings.content), content=str(issue))
                db.session.add(new_issue)
                db.session.commit()

            return jsonify({'message': csv_file}), 200
    except Exception as e:
        return jsonify({'message': 'internal error: %s' % e}), 500
